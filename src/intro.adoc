...
IMSIC units are fully defined in xref:IMSIC.adoc#IMSIC[]. The
format of MSIs used by the RISC-V Advanced Interrupt Architecture is described
in that chapter, xref:IMSIC.adoc#MSIEncoding[].
...
interrupts to virtual machines. Besides xref:IMSIC.adoc#IMSIC[] on
the IMSIC, see xref:VSLevel.adoc#VSLevel[] which specifically
...
their handling are covered mainly in xref:MSLevel.adoc#MSLevel[],
...
to access the IMSIC CSRs (described in xref:CSRs.adoc#CSRs[]
and xref:IMSIC.adoc#IMSIC[]) if Smaia or Ssaia is indicated. Without an
