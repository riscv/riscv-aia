...
xref:AdvPLIC.adoc#AdvPLIC[Advanced Platform-Level Interrupt Controller (APLIC)].
...
IMSIC units are fully defined in xref:IMSIC.adoc#IMSIC[Incoming MSI Controller (IMSIC)]. The
format of MSIs used by the RISC-V Advanced Interrupt Architecture is described
in that chapter, xref:IMSIC.adoc#MSIEncoding[MSI encoding].
...
interrupts to virtual machines. Besides xref:IMSIC.adoc#IMSIC[Incoming MSI Controller (IMSIC)] on
the IMSIC, see xref:VSLevel.adoc#VSLevel[Interrupts for Virtual Machines (VS Level)] which specifically
...
This topic is addressed in xref:IOMMU.adoc#IOMMU[IOMMU Support for MSIs to Virtual Machines].
...
refer to xref:IPIs.adoc#IPIs[Interprocessor Interrupts (IPIs)].
...
their handling are covered mainly in xref:MSLevel.adoc#MSLevel[Interrupts for Machine and Supervisor Levels],
...
xref:IPIs.adoc#IPIs[Interprocessor Interrupts (IPIs)].
...
to access the IMSIC CSRs (described in xref:CSRs.adoc#CSRs[Control and Status Registers (CSRs) Added to Harts]
and xref:IMSIC.adoc#IMSIC[Incoming MSI Controller (IMSIC)]) if Smaia or Ssaia is indicated. Without an
