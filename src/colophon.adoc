...
Resolved some inconsistencies in xref:CSRs.adoc#CSRs[Control and Status Registers (CSRs) Added to Harts] about when to raise
...
Added clarification about the term _IOMMU_ used in xref:IOMMU.adoc#IOMMU[IOMMU Support for MSIs to Virtual Machines].
...
(xref:MSLevel.adoc#majorIntrs[Defined major interrupts and default priorities]).

In xref:MSLevel.adoc#virtIntrs-S[Interrupt filtering and virtual interrupts for supervisor level] added the options for bits 1 and 9 to
be writable in CSR `mvien`, and specified the effects of setting each
of these bits.

Upgraded xref:IOMMU.adoc#IOMMU[IOMMU Support for MSIs to Virtual Machines] ("IOMMU Support") to the _frozen_ state.
...
A comment was added to xref:IPIs.adoc#IPIs[Interprocessor Interrupts (IPIs)] warning about the possible
