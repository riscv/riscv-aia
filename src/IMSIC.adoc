...
latexmath:[$g\times{2}^{E}+A+h\times{2}^{C}$] always equals latexmath:[$\(g\times{2}^{E})$] latexmath:[$A$] latexmath:[$(h\times{2}^{C})$], and
...
|0x40000000 = | Interrupt priority (same as identity)
|===
