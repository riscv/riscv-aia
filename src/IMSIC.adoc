...
external interrupts, as defined by the H extension.
...
When the H extension is implemented and a device is being
...
more on this topic, see xref:IOMMU.adoc#IOMMU[IOMMU Support for MSIs to Virtual Machines].
...
See also xref:AdvPLIC.adoc#AdvPLIC-MSIAddrs[Addresses and data for outgoing MSIs] for the
...
in xref:CSRs.adoc#CSRs[Control and Status Registers (CSRs) Added to Harts]. There is a separate set of CSRs for each
...
As explained in xref:CSRs.adoc#CSRs[Control and Status Registers (CSRs) Added to Harts], registers `miselect` and `mireg` provide indirect access to additional machine-level registers. Likewise for supervisor-level `siselect` and `sireg`, and VS-level `vsiselect` and `vsireg` . In each case, a value of the **_*iselect_** _CSR_ (`miselect`, `siselect` , or `vsiselect)`) in the range 0x70-0xFF selects a register of the
...
The `eidelivery` register affects only whether
an external interrupt appears in a hart's `*ip` register
(MEI or SEI in `mip` or `sip`, or a bit in `hgeip`) and what
...
or on any `*topei` CSR (`mtopei`, `stopei`, or `vstopei`).
...
The value of a `*topei` CSR is not affected by an interrupt file's
`eidelivery` register or by any of `mie`, `sie`, `hie`, `hgeie`, or `vsie`.
...
Changes to the state of an interrupt file are guaranteed
to be reflected in the relevant interrupt-pending bit in CSR
`mip` or `hgeip` eventually, but not necessarily immediately.
