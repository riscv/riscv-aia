...
Local interrupts: counter overflow, asychronous memory buffer
...
(code 13) and asynchronous memory buffer interrupt (code 14) as _local
interrupts_. It is assumed furthermore that any
future definition for reserved interrupt number 15 will also be a
local interrupt. Besides the two RAS interrupts, the AIA additionally
...
Supervisor guest external interrupt +
VS interrupts: external, software, timer +
Local interrupts: counter overflow, asynchronous memory buffer
|39, 19, 38, 37, 18, 36, +
35, 17, 34, 33, 16, 32|
|===
