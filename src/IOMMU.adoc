...
xref:IMSIC.adoc#MSIEncoding[MSI encoding].) This data may be in
...
memory-mapped register `seteipnum_le` (xref:IMSIC.adoc#IMSIC-memRegion[Memory region for an interrupt file]). IMSIC interrupt
...
(xref:IMSIC.adoc#IMSIC-memRegion[Memory region for an interrupt file]), then an IOMMU
