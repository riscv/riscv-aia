...
The IOMMU Architecture Specification provides a detailed description of
...
is explained in Section #sec:IOMMU-identIncomingMSIs[1.4],
...
in Section #sec:virtHartMigration[[sec:virtHartMigration]], when a
...
change. However, because on other systems (not ) the migration of a
...
tables to adjust for the changing physical locations of virtual
...
Section #sec:IOMMU-MRIFs[1.3]. A dedicated MSI page table can easily
support a special table entry format for MRIFs
(Section #sec:IOMMU-MSIPTE-MRIF[1.5.2]) that would be entirely foreign
and difficult to retrofit to any other address-translation data
structures.

=== Translation of addresses for MSIs from devices

To support the delivery of MSIs from I/O devices directly to virtual
...
illustrated in Figure #fig:IOMMU-guestIntrFiles[1.1]. This address
...
Section #sec:IOMMU-identIncomingMSIs[1.4].
...
as regular address translations implies that an address translation
