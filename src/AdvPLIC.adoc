...
interrupt domains (<<AdvPLIC-reg-mmsiaddrcfg>> and
<<AdvPLIC-reg-smsiaddrcfg>>); and
...
|latexmath:[$MSI address = {Base PPN | }({g}<<(HHXS+12))| (h<<LHXS)<<12$]
|===
...
[%autowidth,float="center",align="center",cols="<",grid=none,frame=none]
|===
|latexmath:[$g = ({machine-level hart index\z{>>}LHXW}){ \z{\&} }({2}^{\rm HHXW}-{1})$]
|latexmath:[$h = {machine-level hart index \z{\&} }({2}^{\rm LHXW}-{1})$] +
|latexmath:[$\mbox{MSI address} = \bigl(\,{Base PPN \z{|} }(g{\z{<<}}({HHXS}+{12})){ \z{|} }(h{\z{<<}LHXS}){ \z{|} Guest Index}\,\bigr){\z{<<}12}$]
|===
