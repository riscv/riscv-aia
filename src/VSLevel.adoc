...
As introduced in xref:CSRs.adoc#CSRs[], several hypervisor and VS
...
xref:IMSIC.adoc#IMSIC[] on the IMSIC, along with IMSIC-only CSR `vstopei`.
...
the algorithm of xref:AdvPLIC.adoc#AdvPLIC-MSISync[].
...
in xref:MSLevel.adoc#intrs-S[], interrupt priorities for
...
order defined in xref:MSLevel.adoc#majorIntrs[].
====
...
the manner documented for `stopi` in xref:MSLevel.adoc#stopi[].
