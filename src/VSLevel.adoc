...
As introduced in xref:CSRs.adoc#CSRs[Control and Status Registers (CSRs) Added to Harts], several hypervisor and VS
...
xref:IMSIC.adoc#IMSIC[Incoming MSI Controller (IMSIC)] on the IMSIC, along with IMSIC-only CSR `vstopei`.
...
the algorithm of xref:AdvPLIC.adoc#AdvPLIC-MSISync[Synchronizing interactions between a hart and the APLIC].
...
in xref:MSLevel.adoc#intrs-S[Interrupts at supervisor level], interrupt priorities for
...
order defined in xref:MSLevel.adoc#majorIntrs[Defined major interrupts and default priorities].
====
...
xref:MSLevel.adoc#virtIntrs-S[Interrupt filtering and virtual interrupts for supervisor level].
...
xref:MSLevel.adoc#TableintrPrios-S[Effect of the supervisor-level iprio array on the priorities of interrupts taken in S-mode], except that priority
...
xref:MSLevel.adoc#TablemajorIntrs[The standard major interrupt codes, listed in default priority order], unless `hvictl` fields VTI = 1 and
...
the manner documented for `stopi` in xref:MSLevel.adoc#stopi[Supervisor top interrupt CSR (`stopi`)].
