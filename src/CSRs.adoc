...
xref:IMSIC.adoc#IMSIC[Incoming MSI Controller (IMSIC)] on the IMSIC.

CSR `mtopei` also exists only when an IMSIC is implemented, so is documented in
xref:IMSIC.adoc#IMSIC[Incoming MSI Controller (IMSIC)] along with the indirectly accessed IMSIC
registers.

CSR `mtopi` reports the highest-priority interrupt that is pending and enabled
for machine level, as specified in xref:MSLevel.adoc#mtopi[Machine top interrupt CSR (`mtopi`)].
...
in xref:MSLevel.adoc#virtIntrs-S[Interrupt filtering and virtual interrupts for supervisor level].
...
CSR `stopei` is described with the IMSIC in xref:IMSIC.adoc#IMSIC[Incoming MSI Controller (IMSIC)].

Register `stopi` reports the highest-priority interrupt that is pending and
enabled for supervisor level, as specified in
xref:MSLevel.adoc#stopi[Supervisor top interrupt CSR (`stopi`)].
...
The new hypervisor CSRs in the table (`hvien`, `hvictl` , `hviprio1`, and `hviprio2`) augment `hvip` for injecting interrupts into VS level. The use of these registers is covered in xref:VSLevel.adoc#VSLevel[Interrupts for Virtual Machines (VS Level)] on interrupts for virtual machines.
