...
xref:IMSIC.adoc#IMSIC[] on the IMSIC.

CSR `mtopei` also exists only when an IMSIC is implemented, so is documented in
xref:IMSIC.adoc#IMSIC[] along with the indirectly accessed IMSIC
registers.

CSR `mtopi` reports the highest-priority interrupt that is pending and enabled
for machine level, as specified in xref:MSLevel.adoc#mtopi[].
...
in xref:MSLevel.adoc#virtIntrs-S[].
...
The new hypervisor CSRs in the table (`hvien`, `hvictl` , `hviprio1`, and `hviprio2`) augment `hvip` for injecting interrupts into VS level. The use of these registers is covered in xref:VSLevel.adoc#VSLevel[] on interrupts for virtual machines.
...
The AIA bit controls access to AIA CSRs `siph`, `sieh`, `stopi`, `hidelegh`, `hvien`/`hvienh`, `hviph`, `hvictl`, `hviprio1`/`hviprio1h`, `hviprio2`/`hviprio2h`, `vsiph`, `vsieh`, and `vstopi`, as well as to the supervisor-level interrupt priorities accessed through `siselect` + `sireg` (the `iprio` array of xref:MSLevel.adoc#intrPrios-S[]).
