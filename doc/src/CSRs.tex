...
If one of these bits is zero in \z{mstateen0}, an attempt to access
...
Bit~58 is implemented in \z{mstateen0} only if the hart has an IMSIC.
...
defined also in \z{hstateen0} but concern only the state potentially
...
If one of these bits is zero in \z{hstateen0}, and the same bit is one
in \z{mstateen0}, then an attempt to access the corresponding state
from \mbox{VS-mode} raises a virtual instruction exception.

Bit~58 is implemented in \z{hstateen0} only if the hart has an IMSIC.
Furthermore, even with an IMSIC, bit~58 may (or may not) be read-only
zero in \z{hstateen0} if the IMSIC has no \emph{guest interrupt files}
for guest external interrupts (Chapter~\ref{ch:IMSIC}).
When this bit is zero, a virtual machine is prevented from
...
for \z{hstateen0} but not those for \z{mstateen0},
