...
All \z{hvicontrol} fields together can affect
...
\item
if \z{hvicontrol} fields VTI =~1 and $\mbox{IID} \neq \mbox{9}$:
\begin{tightList}
\item[+]
...
\begin{commentary}
When both \z{hstatus}.VGEIN =~0 and \z{hvicontrol}.VTI =~1, the
absence of an interrupt for VS level can be indicated only by setting
\z{hvicontrol} fields IID =~9 and IPRIO~=~0.
Software might want to use the pair IID =~9, IPRIO~=~0 generally to
represent\/ \emph{no interrupt} in \z{hvicontrol}.
\end{commentary}
...
If bit IPRIOM (IPRIO Mode) of \z{hvicontrol} is zero,
