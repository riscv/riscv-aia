...
are added:  \z{hvien}, \z{hvicontrol}, \z{hviprio1}, \z{hviprio2},
\z{vsiselect}, \z{vsireg}, \z{vstopi}, \z{vsseteipnum},
\z{vsclreipnum}, \z{vsseteienum}, and \z{vsclreienum}.
...
\z{sclaimei}, \z{sseteipnum}, \z{sclreipnum}, \z{sseteienum}, and
\z{sclreienum} is indistinguishable from a true supervisor-level
...
of CSR \z{hvicontrol} (Hypervisor Virtual Interrupt Control), described
...
by setting bit VTI of CSR \z{hvicontrol}, described in the next
subsection.
...
CSR \z{hvicontrol} (Hypervisor Virtual Interrupt Control) provides
...
A hypervisor must use \z{hvicontrol} for any of the following:
\begin{itemize}
...
The format of \z{hvicontrol} is:\nopagebreak
\begin{displayLinesTable}[l@{\quad}l]
bit 30     & VTI \\
bits 27:16 & IID (\WARL) \\
bits 7:0   & IPRIO \\
\end{displayLinesTable}
All other bits of \z{hvicontrol} are reserved and read as zeros.
...
All \z{hvicontrol} fields, VTI, IID, and IPRIO, together can affect
...
bit~9 is one in both \z{vsip} and \z{vsie}, and the \z{topei} register
of the selected guest interrupt file is not zero:
\begin{tightList}
\item[+]
a supervisor external interrupt (code~9) with the priority number
indicated by the guest interrupt file's \z{topei} register;
\end{tightList}

\item
if \z{hstatus}.VGEIN is the valid number of a guest interrupt file,
bit~9 is one in both \z{vsip} and \z{vsie}, and the \z{topei} register
of the selected guest interrupt file is zero:
\begin{tightList}
\item[+]
...
\item
if \z{hstatus}.VGEIN =~0, and \z{hvicontrol} fields IID =~9 and
$\mbox{IPRIO} \neq \mbox{0}$:
\begin{tightList}
\item[+]
...
\item
if \z{hvicontrol} fields VTI =~1, $\mbox{IID} \neq \mbox{9}$, and
$\mbox{IPRIO} \neq \mbox{0}$:
\begin{tightList}
\item[+]
the major interrupt specified by \z{hvicontrol} fields IID and IPRIO.
\end{tightList}
...
If the priority number for the highest-priority candidate is within the
range 1 to 255, IPRIO in \z{vstopi} is that value;
else, IPRIO is saturated to either 1 or 255 in the manner documented
for \z{stopi} in Section~\ref{sec:stopi}, assuming parameter
SMAXIPRIO = 255.
...
operating mode is \mbox{VS-mode} and the SIE bit in CSR \z{vsstatus}
