...
\textbf{%
The parameters used to configure MSI addresses will likely change from
what is currently described here for registers \z{mmsiaddrcfg} and
\z{mmsiaddrcfgh}.%
}
\vspace{3mm}

For machine-level interrupt domains configured in MSI delivery mode
(register \z{domaincfg} has field DM =~1), registers \z{mmsiaddrcfg}
...
MSI delivery mode (\z{domaincfg}.DM is read-only zero), these registers
...
\begin{displayLinesTable}[l@{\quad}l]
bits 31:12 & Low Base PPN (\WARL) \\
bits 10:8  & HXS (\WARL) \\
bit 0      & L \\
\end{displayLinesTable}
All other bits of \z{mmsiaddrcfg} are reserved and read as zeros.
Register \z{mmsiaddrcfgh} has this format:\nopagebreak
\begin{displayLinesTable}[l@{\ \quad}l]
bits 31:0 & High Base PPN (\WARL) \\
\end{displayLinesTable}

Fields High Base PPN from \z{mmsiaddrcfgh} and Low Base PPN from
\z{mmsiaddrcfg} concatenate to form a \mbox{52-bit} Base PPN
(Physical Page Number).
The use of this value and field HXS (Hart Index Shift) for
determining target addresses for MSIs is described later, in
Section~\ref{sec:AdvPLIC-MSIAddrs}.

Fields High Base PPN, Low Base PPN, and HXS are all {\WARL}.
...
In the extreme, the values of all three fields may be entirely
constant, fixed by the implementation.

\begin{commentary}
For most \mbox{32-bit} {\RISCV} systems, and for some \mbox{64-bit}
systems as well, it is expected that \z{mmsiaddrcfgh} will be a
constant zero.
\end{commentary}

If bit~L in \z{mmsiaddrcfg} is set to one, \z{mmsiaddrcfg} and
...
If reset initializes L to one, either the Base PPN and HXS values are
...
\z{mmsiaddrcfgh} may read as zeros, so registers \z{mmsiaddrcfg} and
\z{mmsiaddrcfgh} have only read-only values \z{0x00000001} and zero
respectively.
Any time \z{mmsiaddrcfg} or \z{mmsiaddrcfgh} has a different value
(not \z{0x00000001} or zero respectively), the addresses for
outgoing MSI writes directed to machine level must be derivable
from the visible values of Base PPN and HXS, as specified in
Section~\ref{sec:AdvPLIC-MSIAddrs}.
...
can implement these registers simply as read-only with values
\z{0x00000001} and zero.
...
simply as read-only with values \z{0x00000001} and zero, and a
...
\textbf{%
The parameters used to configure MSI addresses will likely change from
what is currently described here for registers \z{smsiaddrcfg} and
\z{smsiaddrcfgh}.%
}
\vspace{3mm}

For machine-level interrupt domains, registers \z{smsiaddrcfg} and
\z{smsiaddrcfgh} may optionally provide parameters used by child
supervisor-level domains to determine the addresses to write outgoing
MSIs.

Registers \z{smsiaddrcfg} and \z{smsiaddrcfgh} are implemented only if:
...
support MSI delivery mode (\z{domaincfg}.DM is not read-only zero in at
...
bit 0      & L \\
\end{displayLinesTable}
All other bits of \z{smsiaddrcfg} are reserved and read as zeros.
Register \z{smsiaddrcfgh} has this format:\nopagebreak
\begin{displayLinesTable}[l@{\ \quad}l]
bits 31:0 & High Base PPN (\WARL) \\
\end{displayLinesTable}

Fields High Base PPN from \z{smsiaddrcfgh} and Low Base PPN from
\z{smsiaddrcfg} concatenate to form a \mbox{52-bit} Base PPN
(Physical Page Number).
The use of this value and field HXS (Hart Index Shift) for
determining target addresses for MSIs is described later, in
Section~\ref{sec:AdvPLIC-MSIAddrs}.
...
If bit~L in \z{smsiaddrcfg} is set to one, \z{smsiaddrcfg} and
\z{smsiaddrcfgh} are \emph{locked}, read-only.
When L~=~1, the other fields in \z{smsiaddrcfg} and \z{smsiaddrcfgh}
may optionally read as zeros.
In that case, if these other fields were given nonzero values
when L was first set, their values are retained internally by the
PLIC but become no longer visible by reading \z{smsiaddrcfg} and
\z{smsiaddrcfgh}.
...
In the latter case, the other fields in \z{smsiaddrcfg} and
\z{smsiaddrcfgh} may read as zeros, so registers \z{smsiaddrcfg} and
\z{smsiaddrcfgh} have only read-only values \z{0x00000001} and zero
respectively.
Any time \z{smsiaddrcfg} or \z{smsiaddrcfgh} has a different value
(not \z{0x00000001} or zero respectively), the addresses for outgoing
...
If an implementation supports additional forms of reset for the
PLIC besides system reset, it is implementation-defined (or possibly
platform-defined) how these other resets may affect \z{smsiaddrcfg} and
\z{smsiaddrcfgh}.
However, it must not be possible for insufficiently privileged software
to use a localized reset to unlock these registers by changing bit~L
back to zero.
...
cleared if possible.
See Section~\ref{sec:AdvPLIC-pendingBits} for exactly when a pending
bit may be cleared by writing to an \z{in\_setip} register.
...
The pending bit cannot be cleared by a write to an \z{in\_clrip}
register or to \z{clripnum}.
...
\textbf{%
The optional configuration of MSI addresses using registers
\z{mmsiaddrcfg}, \z{mmsiaddrcfgh}, \z{smsiaddrcfg}, and
\z{smsiaddrcfgh} will likely change from what is currently described in
this subsection.%
}
\vspace{3mm}
...
In that case, registers \z{mmsiaddrcfg},
...
\begin{displayLinesTable}
$\mbox{MSI address} =
  \bigl(\mbox{Base PPN \z{|} (Hart Index\z{<<}HXS)}\bigr)\mbox{\z{<<}12}$
\end{displayLinesTable}
The vertical bar (\z{|}) here represents bitwise logical OR, and
\z{<<}$\,k$ represents shifting left by $k$ bits.

For a supervisor-level domain, if MSI target addresses are determined
by the parent domain's \z{smsiaddrcfg} and \z{smsiaddrcfgh} registers,
...
index, the parent domain's \z{smsiaddrcfg} and \z{smsiaddrcfgh}, and
the Guest Index value from \z{target[$i$]}:
\begin{displayLinesTable}
$\mbox{MSI address} =
  \bigl(\mbox{Base PPN \z{|} (machine-level hart index\z{<<}HXS)
      \z{|} Guest Index}\bigr)
    \mbox{\z{<<}12}$
\end{displayLinesTable}
