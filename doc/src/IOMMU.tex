...
If M~=~3, the MSI PTE specifies \emph{translate-R/W mode} for
...
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsection{MSI PTE, translate-R/W mode}

When an MSI PTE has fields V~=~1, C~=~0, and M~=~3
(translate-R/W mode), the PTE's complete format is:\nopagebreak
...
An MSI PTE in translate-R/W mode allows a hypervisor to route an
...
For RV64, the first doubleword of an MSI PTE in translate-R/W mode
...
configured in translate-R/W mode (M~=~3), they might not cache PTEs
